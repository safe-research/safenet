...
- **`getOracleTransactionAttestationByHash` is a breaking public-interface change** (it gains a required `oracleDataHash` argument), so every caller must be updated: the explorer (Phase 4), `examples/attest-safe-tx.ts` (Phase 5), and any `cast`/script usage in docs or `scripts/`. Enumerated across phases; flagged here so a reviewer knows the getter signature change is intentional and its callers are tracked, not missed.
