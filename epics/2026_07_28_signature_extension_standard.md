...
- **Phase 3 — example + docs.** Update `examples/attest-safe-tx.ts` to build the length-prefixed envelope (`... [payload][uint256 len][typeHash]`), and document the envelope + type-hash registry convention in the `SignatureExtension` NatSpec (the canonical spec, per Decisions), with the trailer-format sections of `src/guard/README.md` / `DESIGN.md` pointing to it.

---

## Decisions

The design questions raised during planning are resolved as follows (settled for this epic; the concrete API is finalized in Phase 1):

1. **Library scope — payload-only.** `SignatureExtension` exposes detection and payload extraction, not the owner-signature boundary. The guard does not need it (Safe parses owner signatures from the front independently), and the boundary is derivable as `signatures.length - payloadLength - 64` — documented rather than exposed as a function. A `payloadStart` helper would be added later only if a third-party consumer actually needs it.
2. **Single extension only.** A `signatures` blob carries at most one extension; nesting/stacking is not supported. If a future need arises it becomes a new format (new type hash), keeping the parser simple and unambiguous today.
3. **Spec home — the `SignatureExtension` NatSpec.** The library's own NatSpec is the canonical specification of the envelope and the type-hash registry convention; the guard README/DESIGN reference it rather than restating it, so the spec stays next to the code.
4. **Naming — `SignatureExtension`, `has(...)` + `payload(...)`.** Neutral, descriptive names matching the two roles (presence check vs. extraction).
5. **Explicit fixed-size assertion.** `AttestationTrailer` asserts its payload is exactly 192 bytes and reverts `MalformedAttestationTrailer` on mismatch, rather than relying on `abi.decode` to fail — a clear, guard-owned error.
