...
- `Detector::check` (already exists) builds its `Decision` by running these checks in order and rendering `reason` as `rule.code()` from the first `Err(rule)`, or `approve: true, reason: String::new()` if all pass. No longer human-language message for now — just the bare code.
- Existing three `Detector` unit tests (`detector.rs:26-58`) updated for the new checks.
...
| 2 | `RuleId` taxonomy wired into `Detector`'s checks; base guarantees + reclassified blocklist wired in; charter-mapping doc | 1 | ✅ |
