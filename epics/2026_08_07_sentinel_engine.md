...
**`SafeTransaction` and `Operation` stop being `sol!`-generated.** They were, and that fixed an encoding nobody chose: addresses serialized lower-case, and `sol!` appends a hidden `__Invalid` variant annotated `#[serde(other)]` to every enum, so `"operation": "SELFDESTRUCT"` deserialized _successfully_, and an out-of-range operation byte ABI-decoded to `u8::MAX` rather than erroring. Neither is a defensible thing to publish as a contract, and neither is a defensible thing to hand a verification service quietly.

So `safe_tx::types` declares both by hand, and there is exactly one `SafeTransaction` in the repo's domain code: the same type the checks take, the FSM carries, `validator` puts in a `Packet`, and both services put on the wire. A separate wire-only copy alongside it was considered and rejected — it would mean two spellings of the same transaction, two encodings to keep in step, and a conversion at a boundary with no reason to exist. What writing it by hand buys: checksummed addresses, an `Operation` that is exactly `{CALL, DELEGATECALL}` with no escape hatch, `deny_unknown_fields`, and idiomatic snake_case fields rather than Solidity's. `U256`/`Bytes` keep `alloy`'s encodings (minimal lower-case hex), which are the Ethereum JSON-RPC conventions and what we would have picked anyway. The JSON member names are unchanged from what the `sol!` derive emitted (`rename_all = "camelCase"` reproduces them exactly), so `validator`'s persisted state reads back across the change.

Two things `sol!` was providing get written out explicitly instead of inherited:

- **`Enum.Operation`'s byte encoding**, as `From<Operation> for u8` and `TryFrom<u8> for Operation`, with `0`/`1` spelled out rather than left to a `#[repr(u8)]` cast. The EIP-712 hashing and MultiSend's packed decoder both go through these, and MultiSend's own hand-rolled `0 => CALL, 1 => DELEGATECALL, _ => None` match collapses into the shared one.
- **The event boundary.** `sol!` still generates the ABI event bindings — that is its job, and the encoding there is the contract's to define, not ours — but its `SafeTransaction` copy carries the `__Invalid` variant our `Operation` has nowhere to put, so `From` becomes `TryFrom`. Both services handle it identically where they consume the event: log a warning and ignore the proposal. A transaction whose operation is neither `CALL` nor `DELEGATECALL` reverts on any Safe, so there is nothing to attest to and nothing to vote on. Today that garbage instead flows inward as `__Invalid` and is hashed and checked as `255`.
...
- **A shared `sentinel-engine-api` crate holding the wire types, depended on by both sides.** It would make client/server agreement structural rather than test-enforced, and give a Rust third-party engine something light to depend on. Rejected as over-engineering: it is a `Cargo.toml` and a workspace entry to hold `CheckRequest` and `Verdict` — the part not already shared via `safe-tx` — which is about 25 lines of `struct`/`enum` per side, to solve a drift problem that a golden-JSON fixture on each side already solves, and solves for engines written in any language, which a Rust crate does not. It would also be the third crate in the workspace whose only job is to hold types. The status quo (`dynamic_checker.rs` declaring its own `Request`/`Response`) is already this shape and has caused no trouble. Note the rejection leans on `safe-tx` already owning the transaction: had the 12-field struct been duplicated per side too, a shared crate would have been the better call.
...
- **Generate `openapi.yaml` from the Rust handlers with `utoipa`.** Attractive for drift, and technically available now that the types are hand-written and could derive `ToSchema`. Rejected anyway: it inverts the intended relationship — the YAML is a contract third-party engines implement, not a byproduct of our reference server — and most of the file's value is precisely what a derive cannot express. That `inconclusive` is a success and must not be read as approval, that `rule` is an open set the Charter grows, that addresses are emitted checksummed but accepted in any case, that liveness deliberately lives on another port: all of it would become `#[schema(description = "...")]` attributes stuffed into the types, i.e. the same prose, less readable, reviewed in diffs of Rust rather than of the contract. And the drift it protects against is the cheap half; golden-JSON tests already cover the expensive half (what the types actually emit).
...
`api.rs` holds the `axum` router, the single handler, and the `CheckRequest`/`Verdict`/`ApiError` types (the transaction itself is `safe_tx::types::SafeTransaction`, deserialized directly -- there is no conversion step at the boundary). Behavior:
...
In `safe_tx::types` (Phase 1):

- A fully-populated `SafeTransaction` serializes to exactly the spec's documented JSON, and deserializes back — pinning the camelCase members, hex `Quantity` encoding, `"CALL"`/`"DELEGATECALL"`, and the checksummed address form, character for character. `Default` pins the same encoding's edges: `"0x0"` for zero, `"0x"` for empty `data`.
...
- `deny_unknown_fields` rejects an unknown member; an `operation` of `"SELFDESTRUCT"` fails to deserialize. **This is the case worth a test even though it looks trivial**: it is exactly what the `sol!`-generated type got wrong (`#[serde(other)] __Invalid`), and half the reason the type is hand-written. A regression here is silent.
- `Operation` converts to and from its onchain byte both ways, and rejects `2` and `u8::MAX` — the other half of that reason, and what the EIP-712 hashing and MultiSend's decoder rely on.
...
| ----- | ------------------------------------------------------------------------------------------------------------------ | ---------- | ------ |
| 1     | `safe-tx`: hand-write `SafeTransaction`/`Operation` in place of the `sol!` pair; serde for `RuleId`                | —          | Yes    |
...
### Phase 1 — `safe-tx`: hand-write the transaction types

The one phase that touches all three crates, because replacing a type every one of them uses cannot be done in halves. It lands first because the spec has to describe what these types actually emit.

In `crates/safe-tx`:

- `types.rs`: `SafeTransaction` and `Operation` are declared by hand in place of the `sol!` pair — `deny_unknown_fields`, `rename_all = "camelCase"` (the same member names the derive emitted), addresses emitted checksummed through a `serialize_with` helper and decoded case-insensitively, `U256`/`Bytes` left to `alloy`, snake_case Rust fields, and `Operation` as `Call`/`DelegateCall` with `rename_all = "UPPERCASE"` so the JSON stays `"CALL"`/`"DELEGATECALL"`. Plus `From<Operation> for u8` / `TryFrom<u8> for Operation` and an `InvalidOperation` error. Carries the wire-format tests listed above.
- `bindings.rs` (new): the `sol!` calldata bindings that were sharing `types.rs` (`erc20`, `erc721`, `erc1155`, `multi_send`) move here, which is what the standing `TODO` on `types.rs` asked for and now has a reason to happen: after this change the two halves of that module have genuinely different natures, one hand-written domain type and one set of generated ABI decoders.
...
- `multi_send.rs`: its packed-blob decoder drops its own `0 => CALL, 1 => DELEGATECALL, _ => None` match for `Operation::try_from`.

In `crates/validator` and `crates/sentinel`, the mechanical half is field and variant renames (`chainId` -> `chain_id`, `Operation::CALL` -> `Operation::Call`, and so on) — noting that the `sol!` event bindings keep their Solidity spellings, so only the shared type's own accesses and literals change. The parts that are not mechanical:

- `bindings.rs` in both crates: `From<&bindings::SafeTransaction> for safe_tx::types::SafeTransaction` becomes `TryFrom`.
- `validator`'s `handle_transaction_proposed` / `handle_oracle_transaction_proposed` and the sentinel's `handle_oracle_transaction_proposed`: each converts once, at the top, and ignores the proposal with a `warn` if the operation is not a real one.
- The sentinel converting at the top has a knock-on that Phase 4 was going to do anyway: `StaticChecker::check` now takes `&safe_tx::types::SafeTransaction` rather than the event binding, and the effect gets the already-converted transaction instead of converting a second time. Phase 4 keeps only the FSM half of its original scope.
- `validator`'s EIP-712 hashing drops `tx.operation as u8` for the explicit conversion; the sentinel's `hashing.rs` is untouched, since it works on the event binding rather than the shared type.
...
- `static_checker.rs`: `StaticChecker` implements `Checker` and returns `Verdict`; `Decision` is deleted. Its `check` already takes `&safe_tx::types::SafeTransaction` — Phase 1 moved it off the event binding.
...
- **`CheckRequest`/`Verdict` are duplicated between the two crates on purpose**, with `openapi.yaml` and a golden-JSON fixture on each side as the anti-drift mechanism. The transaction itself is not duplicated — `safe_tx::types` owns it, and it is the same type the checks and the FSM use rather than a wire-only copy of them. If a third Rust consumer of this API ever appears, extracting a shared crate for the envelope is a mechanical follow-up; it is not worth doing for two.
- **Phase 1 changes behavior for a malformed `operation`, deliberately.** Removing `sol!`'s `__Invalid` escape hatch means an out-of-range operation byte can no longer flow inward as `255`; the validator and the sentinel both ignore such a proposal instead of hashing and checking it. This is a fix, not a regression — no Safe can execute that transaction — but it is a behavior change riding along with a refactor, so it is called out here rather than left for a reviewer to notice. It is also unreachable from the current contracts: `Consensus` takes the operation as a Solidity `Enum.Operation`, which reverts on an out-of-range value before an event is ever emitted. The handling exists because the decoder cannot assume that, not because a proposal is expected to hit it.
