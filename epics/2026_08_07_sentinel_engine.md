...
- **`crates/sentinel-engine`** — a new, stateless HTTP service. It accepts a proposed Safe transaction and answers whether it is valid. It holds no request state, has no signer, no database, and submits nothing onchain. Every checker in the repo today (`static_checker.rs`, `cow.rs`, `address_poisoning.rs`) moves here, unchanged. It also holds the hand-authored **`openapi.yaml`** that is the interface's source of truth.
...
**The engine is stateless, synchronous request/response, and untrusted.** One `POST` in, one verdict out. No queue, no callback, no database, no signer, no onchain access other than read-only RPC for its own checks. This keeps the trust story simple: a compromised or buggy engine can only ever make its sentinel vote wrongly (or not at all) — it can never spend the sentinel's bond, sign anything, or corrupt the sentinel's FSM. It also means an operator can restart, rewrite, or replace their engine without touching the process that holds their key and their in-flight requests.
...
**`openapi.yaml` is the contract; each side has its own small types, pinned to it by golden-JSON tests.** The spec lives in `crates/sentinel-engine/`, next to the reference implementation of it. The engine's request/response structs live in its `api.rs`; the sentinel's live in its `engine.rs`, exactly as `dynamic_checker.rs`'s `Request`/`Response` do today. The two sides are not wired together by a shared crate — the spec plus a golden-JSON fixture on each side is what keeps them in agreement, and that is also the only mechanism available to a third-party engine written in another language. A shared Rust types crate was considered and rejected as ceremony for roughly 60 lines (see Alternatives).
...
`Inconclusive` is a **successful** `200` response, not an error: "I could not reach the CoW API" is a legitimate, honest answer that the sentinel must not read as either approval or denial. It is also what the sentinel maps a transport failure or a non-`200` to, so the FSM has exactly one "no trustworthy verdict, drop the request unanswered" path rather than two. `Invalid` carries the `RuleId` as a required field, so the "denied without a recognized rule code" case `dynamic_checker.rs` currently has to detect at runtime becomes unrepresentable on the wire.

**The spec is hand-authored, not generated.** The wire format is not ours to choose freely — `SafeTransaction` is `alloy::sol!`-generated, so its JSON encoding (camelCase members, `U256` as minimal lower-case hex strings, `Operation` as `"CALL"`/`"DELEGATECALL"`) is determined by `alloy`'s serde impls. Generating the spec from the code (`utoipa`) was rejected (see Alternatives): the interesting risk is not "the spec disagrees with our handler", it is "the spec disagrees with what an `alloy` upgrade now emits". Golden-JSON fixtures asserted against the real types catch exactly that, and keep the YAML a reviewable contract rather than build output.
...
- **A shared `sentinel-engine-api` crate holding the wire types, depended on by both sides.** It would make client/server agreement structural rather than test-enforced, and give a Rust third-party engine something light to depend on. Rejected as over-engineering: it is a `Cargo.toml` and a workspace entry for about 60 lines of `struct`/`enum`, to solve a drift problem that a golden-JSON fixture on each side already solves — and that solves it for engines written in any language, which a Rust crate does not. It would also be the third crate in the workspace whose only job is to hold types. The status quo (`dynamic_checker.rs` declaring its own `Request`/`Response`) is already this shape and has caused no trouble.
...
- **Generate `openapi.yaml` from the Rust handlers with `utoipa`.** Attractive for drift, but `SafeTransaction`/`Operation` are `sol!`-generated, so `ToSchema` cannot be derived for them and the interesting half of the schema would be hand-written `impl`s regardless — hand-written Rust describing a hand-written encoding, which is strictly worse to review than hand-written YAML. It also inverts the intended relationship: the YAML is a contract third-party engines implement, not a byproduct of our reference server.
...
info:
  ...
  description: >-
    ...
    transaction here and votes according to the verdict it gets back. The
    engine is stateless: it holds no request state, holds no signing key, and
    submits nothing onchain.
...
paths:
  /v1/check:
    post:
      ...
      summary: Decide whether a proposed Safe transaction is valid.
      parameters:
        - name: x-request-id
          in: header
          required: false
      ...
      responses:
        "200":
          description: >-
            ...
            caller must not read as either approval or denial.
          content:
            application/json:
              schema:
  ...
  schemas:
    ...
    SafeTransaction:
      description: >-
        ...
        `OracleTransactionProposed` event. Unknown members are ignored rather
        than rejected.
      type: object
      required:
        [
    ...
    Verdict:
      ...
      discriminator:
        ...
        mapping:
          ...
          invalid: "#/components/schemas/Invalid"
    ...
    RuleId:
      type: string
      description: >-
        A Safenet Arbitration Charter rule citation. Grown incrementally with
        the checks that give each rule meaning; see `safe_tx::rule::RuleId`.
      enum: ["R-4.1", "R-4.2", "R-4.3", "R-4.4", "R-4.5", "R-4.6"]
    Address:
      type: string
      description: Lower-case, `0x`-prefixed 20-byte address.
...
`CheckRequest` is a wrapper object rather than a bare transaction, so the interface has somewhere to grow (per-request context, hints) without a breaking change. It is the one hand-written struct on the wire, so it is the one place `deny_unknown_fields` can be enforced — `SafeTransaction` is `sol!`-generated and ignores unknown members, which the spec documents rather than lies about.

The spec's `example`s are the same JSON literals both crates' golden tests assert against, with a comment on each side pointing at the file.

### `crates/sentinel-engine`

New binary crate, `argh`-parsed `--config-file`/`--version` options matching `crates/sentinel/src/main.rs`. Dependencies: `alloy`, `argh`, `axum`, `reqwest`, `safe-tx`, `safenet-core` (observability only), `serde`, `thiserror`, `tokio`, `toml`, `tracing`, `url`. Notably **not** `sqlx` and **no** `Signer` — the engine has no database and no key.
...
`api.rs` holds the `axum` router, the single handler, and the `CheckRequest`/`Verdict`/`ApiError` types. Behavior:

- `POST /v1/check` -> run the checker chain, `200` with a `Verdict`.
- Body-size limit left at `axum`'s `DefaultBodyLimit` (2 MiB), which is far above the largest plausible MultiSend calldata blob.
- Malformed body, wrong content type, or `operation` outside `{CALL, DELEGATECALL}` -> `400`/`415` with an `ApiError`; a checker panic or unexpected internal failure -> `500`. A _dependency_ failure (unreachable RPC, unreachable CoW API) is not an error: it is `200 Inconclusive`.
...
Both crates carry the same golden-JSON assertions against the spec's documented examples — the engine for what it accepts and emits, the sentinel for what it sends and parses:

- A fully-populated `CheckRequest` serializes to exactly the documented JSON, and deserializes back — pinning the camelCase members, hex `Quantity` encoding (including `"0x0"` for zero and `"0x"` for empty `data`), lower-case addresses, and `"CALL"`/`"DELEGATECALL"`.
- Each of the three `Verdict` variants round-trips to and from its documented JSON.
- `deny_unknown_fields` rejects an unknown top-level member of `CheckRequest`.
- An unrecognized `rule` string fails to deserialize.
- **`Operation` needs an explicit guard.** `alloy::sol!` appends a hidden `__Invalid` variant annotated `#[serde(other)]` whenever serde is derived, so an unknown `operation` string deserializes _successfully_ into `__Invalid` rather than failing. The engine must reject a transaction whose `operation` is neither `CALL` nor `DELEGATECALL` with a `400`; a test covers it at the engine's HTTP boundary.

### `crates/sentinel` after the split

`src/` loses `checker.rs`, `static_checker.rs`, `cow.rs`, `address_poisoning.rs` and `dynamic_checker.rs`; it gains `engine.rs`, holding the client, its wire types, and the `Verdict` the FSM resumes on:
...
- **`crates/sentinel`** adapts `dynamic_checker.rs`'s existing one-shot-`TcpListener` tests onto `engine.rs`: valid, invalid with a cited rule, inconclusive, unrecognized rule code, non-`200`, unreachable endpoint, and timeout.
...
| ----- | ------------------------------------------------------------------------------------------------------------------ | ---------- | ------ |
| 1     | `safe-tx`: `Serialize`/`Deserialize` for `RuleId` via its Charter code                                             | —          | Yes    |
...
### Phase 1 — `safe-tx`: serde for `RuleId`

`crates/safe-tx/src/rule.rs` only. Manual `Serialize` (`serialize_str(self.code())`) and `Deserialize` (`from_code`, with an error listing the valid codes) rather than derives, so the Charter citation stays the single wire form and `code`/`from_code` stay the single place the mapping lives. Extends the existing round-trip test to also cover JSON. No other crate changes.

### Phase 2 — `crates/sentinel-engine`: the service and its contract

New crate: `Cargo.toml`, `openapi.yaml`, `main.rs`, `config.rs` (`bind_address` + `observability` only at this point), `api.rs` (router, handler, wire types, golden-JSON tests, router tests), `Dockerfile`, `Dockerfile.dockerignore`, plus the workspace `Cargo.toml` and `.github/workflows/docker.yml` entries.
...
`AddressPoisoningChecker` is deliberately the first checker moved, because it is the only one that never returns `Invalid`: it either approves off unforgeable evidence of a prior interaction, or abstains. While it is dormant (Phases 5-6), the sentinel's chain simply falls through to the next checker more often, and no vote it casts differs from what it would have cast before.
...
- **The wire types are duplicated between the two crates on purpose**, with `openapi.yaml` and a golden-JSON fixture on each side as the anti-drift mechanism. If a third Rust consumer of this API ever appears, extracting a shared types crate is a mechanical follow-up; it is not worth doing for two.
