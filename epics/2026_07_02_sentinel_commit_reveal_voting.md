...
`keccak256(abi.encodePacked(approve, salt))`. Binding `requestId` closes a specific leak: a sentinel
that reuses the same `(approve, salt)` pair across two concurrent requests would otherwise produce
**identical commit hashes** for both — visibly correlating its vote across requests before either is
revealed, which defeats blindness just as effectively as a plaintext vote would for the second
request. Binding `msg.sender` is defense-in-depth (each commitment is already keyed by sentinel
...
- `finalize()` computes `unrevealedBond = totalCommittedBond - totalApproveBond - totalDenyBond`
  ...
  distribution logic is introduced.
...
- `claim()` must reject any commitment where `revealed == false` (new `NotRevealed` error). Without
...
- `finalize()`: after computing `(newState, refundFee, unrevealedBond)`, transfer `unrevealedBond` to
  `ARBITRATOR` whenever it is non-zero, in addition to existing per-outcome behavior.
...
- `state.rs`: `RequestStatus` gains `Revealing` (commit confirmed, waiting for the reveal window to
  ...
  `deadline`; the salt is **not** stored (see [Architecture Decision](#architecture-decision)).
  `SentinelActionKind::CommitApprove`/`CommitDeny` collapse into `Commit { id, hash }`; new
  `Reveal { id, approve, salt }`.
- `service.rs`:
  ...
  - `handle_committed` (our own `Committed` event) moves `Pending → Revealing` (renamed from
    `Committed`) — it does **not** yet emit the `Reveal` action, since revealing before
    `commitDeadline` is invalid onchain.
  - `handle_block_advance` gains the phase transition V1 didn't need: once `block > commitDeadline`
    for a `Revealing` request, emit the `Reveal` action (deriving the salt on the fly) and keep
    status `Revealing` until our own `Revealed` event confirms it (`Revealing → Revealed`); once
    `block > revealDeadline` (or earlier — informational only, `finalize` is still a separate action
    keyed off the deadline) for a `Revealed` request, emit `Finalize` exactly as V1 did from
    `Committed`.
