	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" role="presentation">
			<div
				role="alertdialog"
				aria-modal="true"
				aria-labelledby="rpc-warning-title"
				aria-describedby="rpc-warning-desc"
				className="bg-surface-1 border border-warning-outline rounded-card p-6 max-w-md mx-4 shadow-elevated"
			>
				<h2 id="rpc-warning-title" className="text-base font-semibold text-warning mb-2">
				...
				<p id="rpc-warning-desc" className="text-sm text-sub-title mb-6">
					Only open links from sources you trust. A malicious RPC can return false data about transactions and balances.
				</p>
				{/* biome-ignore lint/a11y/noAutofocus: focus is intentional for alertdialog accessibility */}
				<Button autoFocus variant="primary" onClick={() => setDismissed(true)} className="w-full justify-center">
