...
/** Returns `0x` + first 8 hex chars + `…` + last 8 hex chars. Returns the full hash if it is shorter than 18 characters. */
...
export const formatBlockAge = (blockDiff: bigint, chain: ChainInfo, now?: number): string => {
	const blockTimeMs = chain.blockTime ?? 12_000;
	const seconds = Number(blockDiff) * (blockTimeMs / 1000);
	if (seconds < 60) return `${Math.floor(seconds)}s ago`;
	if (seconds < 3600) return `${Math.floor(seconds / 60)}m ago`;
	const date = new Date((now ?? Date.now()) - seconds * 1000);
