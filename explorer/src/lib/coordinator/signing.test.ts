...
// Full mock provider for signing-progress tests (Sign event present).
...
describe("loadLatestAttestationStatus — participation fields", () => {
	beforeEach(() => {
		vi.resetModules();
	});
	...
	it("populates committed from SignRevealedNonces events", async () => {
		...
		expect(result?.committed).toEqual([{ address: PARTICIPANT_A, block: 2n }]);
		expect(result?.signed).toEqual([]);
	});

	it("populates signed from SignShared events", async () => {
		...
		const sharedLog = { ...makeSharedLog(SID, PARTICIPANT_A, "0x3"), data: sharedData };
		const provider = makeFullProvider({ progressLogs: [sharedLog] });
		const load = await loadModule();
