...
//! as claiming only [`Aspect::Refund`] — the engine still needs another
...
use crate::{
    ...
    engine::{Aspect, Coverage, SafeTransaction},
};
...
impl Checker for RefundChecker {
    ...
    /// reinterpreted as covering only [`Aspect::Refund`] — the recipient's
    ...
    async fn check(&self, transaction: &SafeTransaction, context: &CheckContext) -> Assessment {
        ...
        match self.0.check(&refund, context).await {
            Assessment::Secure { .. } => Assessment::Secure {
                coverage: Coverage::of(&[Aspect::Refund]),
            },
