...
impl Account {
    ...
    pub fn new(private_key: B256) -> Result<Self, k256::ecdsa::Error> {
...
mod tests {
    ...
    fn can_sign_transactions() {
        let account = Account::new(keccak256("top secret key")).unwrap();
        let tx = TxEip1559::default();
        let signed = account.sign_transaction(tx.clone()).unwrap();

        let decoded =
            Signed::<TxEip1559, Signature>::decode_2718_exact(&signed.into_raw()).unwrap();
