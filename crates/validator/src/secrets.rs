...
impl SecretStore {
    ...
    /// owning `group`, and returns its Merkle root.
    pub async fn register_nonces_chunk(
        ...
        sqlx::query("INSERT INTO nonces_chunks (root, group_id) VALUES (?, ?)")
            .bind(key(root))
            .bind(key(group))
            .execute(&mut *tx)
            .await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn nonce_chunks_are_stored_by_root() {
        let store = store().await;
        let root = store
            .register_nonces_chunk(GROUP, nonce_chunk(11))
            .await
            .unwrap();
        let other = store
            .register_nonces_chunk(GROUP, nonce_chunk(12))
            .await
            .unwrap();
    ...
    async fn use_nonce_removes_it_permanently() {
        let store = store().await;
        let root = store
            .register_nonces_chunk(GROUP, nonce_chunk(4))
            .await
            .unwrap();
    ...
    async fn nonces_reveal_is_non_consuming() {
        ...
        assert!(store.take_nonce(root, 1).await.unwrap().is_some());

        // Once taken, there is no nonce left to reveal, nor is an out-of-range
        // offset revealable.
        assert!(store.nonces_reveal(root, 1).await.unwrap().is_none());
    ...
    async fn prune_group_nonces_removes_trees_and_nonces() {
        let store = store().await;
        let root = store
            .register_nonces_chunk(GROUP, nonce_chunk(2))
            .await
            .unwrap();
        let other = store
            .register_nonces_chunk(B256::repeat_byte(0xb2), nonce_chunk(2))
            .await
            .unwrap();

        store.prune_group_nonces(GROUP).await.unwrap();
        // The cascade removed this group's nonces without touching another
        // group's root.
        assert!(store.take_nonce(root, 0).await.unwrap().is_none());
        assert!(store.take_nonce(other, 0).await.unwrap().is_some());
    }
}
