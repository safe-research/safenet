...
pub mod consensus {
    ...
    // `TryFrom` converts one into the other at the point an event is consumed,
    ...
    //
    // Fallible because `sol!` decodes an out-of-range `Enum.Operation` byte
    // into its hidden `__Invalid` variant instead of erroring, and
