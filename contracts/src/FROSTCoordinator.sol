...
contract FROSTCoordinator {
     ...
     * @dev Per-participant decline and reveal state is tracked in `FROSTNonceCommitmentSet` via two
     *      boolean flags per sequence, enforcing that each participant either reveals nonces (at most
     *      once) or declines (at most once) — the two transitions are mutually exclusive.
