...
**Final: Confirmed, High, 99 (E1), canonical.** Rediscovered by run 2 as `F2-CORE-002` (CONFIRMS, E1 at the `EventWatcher` level: three bloom-rejected `[]`, then an unverified empty update accepted; three 429s spend the same budget). Run 2 rated it Medium 90 (degraded RPC is an A4 condition, not attacker input); the combined report carries High 99 — the Phase 8 A/B drove the loss to an on-chain balance change and the operator had done exactly what the handbook prescribes (`state/run2/reconciliation/core.md` §1.1). Run 2 did **not** rediscover the sibling `F-CORE-012` (bloom-equality blind spot), which stays beside this finding as the second route to the same outcome. Still valid at `fe9e84c` (`events.rs` unchanged). Counterparts: `F2-CORE-002`; run-2 NEW `F2-CORE-003` covers the default (non-client-filtered) path's absence of any completeness check.
