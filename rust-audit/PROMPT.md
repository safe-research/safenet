...
- **Delivery.** No intermediate commits: run-2 output accumulates on disk and lands on the existing PR as **exactly two commits** once the review is complete — one with every review file, then `report/REPORT.md` rewritten for the combined result **on its own**, so reviewers can diff it in isolation. Never a force push, never a commit outside `rust-audit/`, no co-author trailer, Prettier-clean and CI-verified before each push.
