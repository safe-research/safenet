...
1. **Check pattern** — each check implements the `Checker` trait (see `checkers/base.rs`'s `Checker` impl for the reference shape: `name()` + `async fn check(&self, transaction) -> Verdict`), lives under `crates/sentinel-engine/src/checkers/`, and is registered both in that module's exports (`checkers/mod.rs`) and wherever the engine composes its checker chain (`main.rs`'s `SentinelEngine::new` call). A checker with no opinion must return `Verdict::Abstain` so the next checker in the chain gets a turn — it must never invent a `secure`/`insecure` verdict just to answer.
