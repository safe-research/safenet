...
1. **Check pattern** — each check implements the `Checker` trait (see `checkers/base.rs`'s `Checker` impl for the reference shape: `name()` + `async fn check(&self, proposal, context) -> Assessment`), lives under `crates/sentinel-engine/src/checkers/`, and is registered both in that module's exports (`checkers/mod.rs`) and wherever the engine composes its checker chain (`main.rs`'s `SentinelEngine::new` call). A checker with no opinion must return `Assessment::Abstain` so the next checker in the chain gets a turn — it must never invent a `Secure`/`Insecure` assessment just to answer. `context: &CheckContext` (`engine/mod.rs`) carries caller-supplied hints outside the transaction itself (currently `block`, a `BlockLabel` — a block number or `latest` — from the request's required `block` field, for checks that read RPC-derived state, and `proposal_timestamp`, from the optional `x-proposal-timestamp` header; see `docs/sentinel-engine.md`); most checks ignore it and should just take-and-drop the parameter.
...
5. **Coverage claims** — an `Assessment::Secure` must state the `Coverage` (`crates/sentinel-engine/src/engine/coverage.rs`) it vouches for, and only that: the engine composes `secure` from the union of every check's claim, so a claim wider than what the check actually examined lets an unexamined aspect through as if it had been looked at. There is no default coverage and none should be added — a check that forgets to narrow its claim must not silently affirm the whole transaction. A deny-only check (one with no affirming path) has no claim to write at all. Coverage is per call (`Vec<AspectSet>`, indexed parallel to `Proposal::calls`, plus one `refund` flag for the whole proposal) — claim only the call indices your check actually examined, via `Coverage::calls`/`Proposal::checked` and `Coverage::refund`/`Proposal::refund_checked`, not the transaction as a whole. See [Verdict Composition](./docs/sentinel-engine.md#verdict-composition) for the full model.
6. **Batches are pre-flattened** — a check receives a `Proposal`, not a bare transaction: `proposal.transaction` carries identity and refund fields, and `proposal.calls` is what to evaluate — one entry for an ordinary transaction, one per packed sub-call in execution order for a recognized MultiSend batch. Evaluate `proposal.calls`; don't re-derive a batch yourself (no more decoding MultiSend calldata inside a checker). See [Verdict Composition](./docs/sentinel-engine.md#verdict-composition) for what gets flattened and why.
