...
Per default, the `STAKER_ADDRESS` will receive all validator rewards including commission. In order to have commission be distributed to another beneficiary, validators must set a delegate on the [DelegateRegistry](https://etherscan.io/address/0x469788fE6E9E9681C6ebF3bF78e7Fd26Fc015446) with `id = keccak256(toHex("Safenet Beta validator commission beneficiary"))`
...
function: setDelegate
    ...
    delegate: 0x...  // beneficiary address

```

Please inform the Safe team in case you intend to do this so we can make sure everything is accounted properly. 

Once executed, `STAKER_ADDRESS` will only receive rewards on its own stake. `beneficiary` will onlyreceive the commission.
